...
def s1_preproc(params):
    ...
    if (PLATFORM_NUMBER=='A' or PLATFORM_NUMBER=='B' ):
        print("Selecting PLATFORM ", PLATFORM_NUMBER)
        s1 = s1.filter(ee.Filter.eq('platform_number', PLATFORM_NUMBER))
   

    if (ORBIT_NUM != None):
        print("Selecting ORBIT ", ORBIT_NUM)
    ...
    # select orbit
    if (ORBIT != 'BOTH'):
        print("Selecting ORBIT ", ORBIT)
        s1 = s1.filter(ee.Filter.eq('orbitProperties_pass', ORBIT))

    # select polarization
    if (POLARIZATION == 'VV'):
        print("Selecting POLARIZATION ", POLARIZATION)
    ...
    if (APPLY_SPECKLE_FILTERING):
        ...
        elif (SPECKLE_FILTER_FRAMEWORK == 'MULTI'):
    ...
    if (FORMAT == 'DB'):
        print('Convertion to Decibels is completed')
        s1_1 = s1_1.map(helper.lin_to_db)
        
        
    #clip to roi
    if (CLIP_TO_ROI):
        print('Clip to ROI is completed')
